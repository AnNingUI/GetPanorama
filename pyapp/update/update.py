...
class AppUpdate:
    ...
    def run(self):
        ...
        if resCheck['code'] == 0:
            ...
            if not resApp['status']: # type: ignore
                return {'code': -2, 'msg': '下载程序包失败: ' + resApp['msg']} # type: ignore
            else:
                return {'code': 0, 'msg': '下载程序包成功', 'downloadPath': resApp['downloadPath']} # type: ignore
        else:
            return resCheck
    ...
    def __getNewInfo(self):
        ...
        try:
            # 3秒后连接超时，3秒后读取超时
            r = httpx.get(Config.appUpdateUrl, timeout=(3, 3)) # type: ignore
            resJson = r.json()
        ...
        try:
            with open(downloadPath, "wb") as f:
                with httpx.Client(follow_redirects=True) as client:
                    with client.stream("GET", url, timeout=(5, 3600)) as r: # type: ignore
                        downloadSize = 0
                        infoPy2jsDict = dict()
                        # 每块 1KB
