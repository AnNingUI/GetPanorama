...
class Config:
    ...
    # 程序基础配置信息
    ##
    appName = 'GetPanorama'  # 应用名称
    appNameEN = 'get_panorama'    # 应用名称-英文（用于生成缓存文件夹，必须是英文）
    appVersion = "V0.0.1"  # 应用版本号
    appDeveloper = "AnNingUI_AND_QINGFONG"  # 应用开发者
    appBlogs = ""  # 个人博客
    appPackage = 'vip.lrcm'    # 应用包名，用于在本地电脑生成 vip.pangao.ppx 唯一文件夹
    appUpdateUrl = ''    # 获取程序更新信息 https://api.github.com/repos/pangao1990/ppx/releases/latest
    ...
    def getDir(self):
        ...
        if Config.appSystem == 'Darwin':
            ...
            # win系统
            downloadDir = os.path.join(os.getenv('USERPROFILE'), 'Downloads') # type: ignore
            appDataDir = os.path.join(os.getenv('APPDATA'), Config.appPackage+'.'+Config.appNameEN) # type: ignore
        elif Config.appSystem == 'Linux':
            # linux系统
            downloadDir = os.path.join(os.getenv('HOME'), 'Downloads') # type: ignore
            appDataDir = os.path.join(os.getenv('HOME'), '.'+Config.appPackage+'.'+Config.appNameEN) # type: ignore
